...
/* Add function for expand bitmap size */
struct bitmap *bitmap_expand(struct bitmap *bitmap, int size)
{
	int i, bm_size;
	struct bitmap *n_bitmap;
	bm_size = bitmap_size(bitmap);
	bm_size += size;
	n_bitmap = bitmap_create(bm_size);
	for(i = 0; i < (bm_size - size); i++)
	{
		bitmap_set(n_bitmap,i,bitmap_test(bitmap,i));
	}
	return n_bitmap;
}
